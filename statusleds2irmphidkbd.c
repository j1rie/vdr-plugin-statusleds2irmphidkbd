...
static bool open_stm32(const char *devicename) {
	...
	if (stm32fd == -1) {
		dsyslog("statusleds2irmphidkbd: error opening stm32 device: %s\n",strerror(errno));
...
static void read_stm32() {
	...
	if (retVal < 0) {
		dsyslog("statusleds2irmphidkbd: read error\n");
		printf("read error\n");
        } /*else {
                printf("read %d bytes:\n\t", retVal);
...
static void write_stm32() {
	...
	if (retVal < 0) {
		dsyslog("statusleds2irmphidkbd: write error\n");
		printf("write error\n");
        } /*else {
...
// --- cMenuSetupStatusLeds2irmphidkbd -------------------------------------------------------

class cMenuSetupStatusLeds2irmphidkbd : public cMenuSetupPage {
private:
  int iNewOnDuration;
  int iNewOffDuration;
...
public:
  cMenuSetupStatusLeds2irmphidkbd(void);
  };

cMenuSetupStatusLeds2irmphidkbd::cMenuSetupStatusLeds2irmphidkbd(void)
{
...
void cMenuSetupStatusLeds2irmphidkbd::Set(void)
{
  int current = Current();
  Clear();
  ...
  // Add ioctl() options
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmphidkbd$On time (100ms)"), &iNewOnDuration, 1, 99));
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmphidkbd$On pause time (100ms)"), &iNewOnPauseDuration, 1, 99));
  Add(new cMenuEditIntItem( tr("Setup.StatusLeds2irmphidkbd$Off time (100ms)"), &iNewOffDuration, 1, 99));

  SetCurrent(Get(current));
}

eOSState cMenuSetupStatusLeds2irmphidkbd::ProcessKey(eKeys Key)
{
...
void cMenuSetupStatusLeds2irmphidkbd::Save(void)
{
...
void cMenuSetupStatusLeds2irmphidkbd::Store(void)
{
  Save();
...
// --- cPluginStatusLeds2irmphidkbd ----------------------------------------------------------

cPluginStatusLeds2irmphidkbd::cPluginStatusLeds2irmphidkbd(void)
{
...
bool cPluginStatusLeds2irmphidkbd::ProcessArgs(int argc, char *argv[])
{
...
{
    dsyslog("statusleds2irmphidkbd: Thread started (pid=%d)", getpid());
...
void cPluginStatusLeds2irmphidkbd::Stop(void)
{
...
cMenuSetupPage *cPluginStatusLeds2irmphidkbd::SetupMenu(void)
{
  ...
  return new cMenuSetupStatusLeds2irmphidkbd;
}

bool cPluginStatusLeds2irmphidkbd::SetupParse(const char *Name, const char *Value)
{
...
VDRPLUGINCREATOR(cPluginStatusLeds2irmphidkbd); // Don't touch this!
